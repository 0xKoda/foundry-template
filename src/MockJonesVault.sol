...
abstract contract MockVault is ERC20("Mock cERC20 Strategy", "cERC20", 18), IERC4626{
   ...
   function setApprovedStrategy(address strategy) internal returns (bool) {
     Strategies[strategy]=true;
   }
    //removes an approved strategy
    function removeApprovedStrategy(address strategy) internal returns (bool) {
...
/// Function to deposit into strategy contract
 function depositStrategy(address strategy, uint256 amount) internal returns (uint256 balance) {
    ...
    //Function to withdraw from Strategy
     function withdrawFromStrategy(address strategy, uint256 amount) internal onlyAdmin returns (uint256) {
    ...
    //Custom logic for querying if jones strategy is approved
    ...
    //Custom logic for querying jone strategy window
    function getWindow() external view returns (uint256) {
      // some logic to check if strategy has locked
      //returns ether (0 locked, 1 unlocked, 2 Accepting deposits)
    }
